...
enum { QFirebirdChunkSize = SHRT_MAX / 2 };

#if defined(FB_API_VER) && FB_API_VER >= 20
static bool getFirebirdError(QString& msg, const ISC_STATUS* status, ISC_LONG &sqlcode, QTextCodec *tc)
#else
static bool getFirebirdError(QString& msg, ISC_STATUS* status, ISC_LONG &sqlcode, QTextCodec *tc)
#endif
{
...
static QVariant::Type qFirebirdTypeName(int iType, bool hasScale)
{
    ...
    qWarning("qFirebirdTypeName: unknown datatype: %d", iType);
    return QVariant::Invalid;
}

static QVariant::Type qFirebirdTypeName2(int iType, bool hasScale)
{
    switch(iType & ~1) {
    case SQL_VARYING:
    case SQL_TEXT:
...
public:
    QFirebirdDriverPrivate() : QSqlDriverPrivate(), firebird(0), trans(0), tc(0) { dbmsType = QSqlDriver::Interbase; }
    ...
    bool isError(const char *msg, QSqlError::ErrorType typ = QSqlError::UnknownError)
    {
        ...
        QString imsg;
        ISC_LONG sqlcode;
        if (!getFirebirdError(imsg, status, sqlcode, tc))
            return false;
...
bool QFirebirdResultPrivate::writeBlob(int i, const QByteArray &ba)
{
    ...
    if (!isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Unable to create BLOB"),
        ...
        int i = 0;
        while (i < ba.size()) {
            isc_put_segment(status, &handle, qMin(ba.size() - i, int(QFirebirdChunkSize)),
                            const_cast<char*>(ba.data()) + i);
            if (isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Unable to write BLOB")))
                return false;
            i += qMin(ba.size() - i, int(QFirebirdChunkSize));
        }
    }
...
QVariant QFirebirdResultPrivate::fetchBlob(ISC_QUAD *bId)
{
    ...
    QByteArray ba;
    int chunkSize = QFirebirdChunkSize;
    ba.resize(chunkSize);
    int read = 0;
...
bool QFirebirdResultPrivate::writeArray(int column, const QList<QVariant> &list)
{
    Q_Q(QFirebirdResult);
    QString error;
    ...
    if (!createArrayBuffer(ba.data(), list,
                           qFirebirdTypeName(desc.array_desc_dtype, inda->sqlvar[column].sqlscale < 0),
                           0, &desc, error, tc)) {
...
bool QFirebirdResult::exec()
{
    ...
    if (d->inda) {
        ...
        if (values.count() > d->inda->sqld) {
            qWarning("QFirebirdResult::exec: Parameter mismatch, expected %d, got %d parameters",
        ...
        for (i = 0; i < values.count(); ++i) {
            ...
            case SQL_ARRAY:
                    ...
                    break;
            default:
                    qWarning("QFirebirdResult::exec: Unknown datatype %d",
    ...
    if (ok) {
        if (colCount() && d->queryType != isc_info_sql_stmt_exec_procedure) {
            ...
            if (d->isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Unable to close statement")))
                return false;
            cleanup();
        }
...
bool QFirebirdResult::gotoNext(QSqlCachedResult::ValueCache& row, int rowIdx)
{
    ...
    for (int i = 0; i < d->sqlda->sqld; ++i) {
        ...
        if ((d->sqlda->sqlvar[i].sqltype & 1) && *d->sqlda->sqlvar[i].sqlind) {
            // null value
            QVariant v;
            v.convert(qFirebirdTypeName2(d->sqlda->sqlvar[i].sqltype, d->sqlda->sqlvar[i].sqlscale < 0));
...
QSqlRecord QFirebirdResult::record() const
{
    ...
    for (int i = 0; i < d->sqlda->sqld; ++i) {
        ...
        QSqlField f(QString::fromLatin1(v.aliasname, v.aliasname_length).simplified(),
                    qFirebirdTypeName2(v.sqltype, v.sqlscale < 0),
...
QSqlRecord QFirebirdDriver::record(const QString& tablename) const
{
    ...
    while (q.next()) {
        ...
        QSqlField f(q.value(0).toString().simplified(), qFirebirdTypeName(type, hasScale), tablename);
        if(hasScale) {
...
QSqlIndex QFirebirdDriver::primaryIndex(const QString &table) const
{
    ...
    while (q.next()) {
        QSqlField field(q.value(1).toString().simplified(),
                        qFirebirdTypeName(q.value(2).toInt(), q.value(3).toInt() < 0),
                        tablename);
...
QVariant QFirebirdDriver::handle() const
{
    ...
    return QVariant(qRegisterMetaType<isc_db_handle>("isc_db_handle"), &d->firebird);
}
...
bool QFirebirdDriver::subscribeToNotification(const QString &name)
{
    Q_D(QFirebirdDriver);
    if (!isOpen()) {
        qWarning("QFirebirdDriver::subscribeFromNotificationImplementation: database not open.");
        return false;
    }

    if (d->eventBuffers.contains(name)) {
        qWarning("QFirebirdDriver::subscribeToNotificationImplementation: already subscribing to '%s'.",
            qPrintable(name));
        return false;
    }
...
bool QFirebirdDriver::unsubscribeFromNotification(const QString &name)
{
    Q_D(QFirebirdDriver);
    if (!isOpen()) {
        qWarning("QFirebirdDriver::unsubscribeFromNotificationImplementation: database not open.");
        return false;
    }

    if (!d->eventBuffers.contains(name)) {
        qWarning("QFirebirdDriver::QFirebirdSubscriptionState not subscribed to '%s'.",
            qPrintable(name));
        return false;
    }
    ...
    for (i = d->eventBuffers.constBegin(); i != d->eventBuffers.constEnd(); ++i) {
        ...
        if (counts[0]) {
            ...
            if (Q_UNLIKELY(status[0] == 1 && status[1])) {
                qCritical("QFirebirdDriver::qHandleEventNotification: could not resubscribe to '%s'",
                    qPrintable(i.key()));
            }
