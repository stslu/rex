...
Q_STATIC_LOGGING_CATEGORY(lcFirebird, "qt.sql.firebird")
...
#define FIREBIRD_INT128_SUPPORTED
#endif
...
constexpr qsizetype QFirebirdChunkSize = SHRT_MAX / 2;
...
static bool getFirebirdError(QString& msg, const ISC_STATUS* status, ISC_LONG &sqlcode)
{
...
static void initDA(XSQLDA *sqlda)
{
    for (int i = 0; i < sqlda->sqld; ++i) {
...
#ifdef FIREBIRD_INT128_SUPPORTED
        case SQL_INT128:
#endif
#endif
        case SQL_TYPE_TIME:
        ...
        default:
            ...
            sqlvar.sqldata = 0;
            qCWarning(lcFirebird, "initDA: unknown sqltype: %d", sqltype);
            break;
        }
        if (sqlvar.sqltype & 1) {
...
static QMetaType::Type qFirebirdTypeName(int iType, bool hasScale)
{
    switch (iType) {
    case blr_varying:
    case blr_varying2:
    ...
    qCWarning(lcFirebird, "qFirebirdTypeName: unknown datatype: %d", iType);
    return QMetaType::UnknownType;
}

static QMetaType::Type qFirebirdTypeName2(int iType, bool hasScale)
{
    switch(iType & ~1) {
    case SQL_VARYING:
    case SQL_TEXT:
...
static constexpr auto s_firebirdBaseDate = QDate::fromJulianDay(2400001);

static inline ISC_TIMESTAMP toTimeStamp(const QDateTime &dt)
{
    ISC_TIMESTAMP ts;
    ts.timestamp_time = dt.time().msecsSinceStartOfDay() * 10;
    ts.timestamp_date = s_firebirdBaseDate.daysTo(dt.date());
    return ts;
}
...
static inline QDateTime fromTimeStamp(const char *buffer)
{
    ...
    const QDate d = s_firebirdBaseDate.addDays(timebuf->timestamp_date);
...
static inline QDateTime fromTimeStampTz(const char *buffer)
{
    ...
    const QDate d = s_firebirdBaseDate.addDays(timebuf->utc_timestamp.timestamp_date);
...
static inline ISC_TIMESTAMP_TZ toTimeStampTz(const QDateTime &dt)
{
    ...
    ts.utc_timestamp.timestamp_date = s_firebirdBaseDate.daysTo(dtUtc.date());
...
{
    return s_firebirdBaseDate.daysTo(t);
 }
...
static inline QDate fromDate(const char *buffer)
{
    ...
    return s_firebirdBaseDate.addDays(int(tsbuf->timestamp_date));
}
...
struct QFirebirdEventBuffer {
    ...
    enum QFirebirdSubscriptionState { Starting, Subscribed, Finished };
    QFirebirdSubscriptionState subscriptionState;
};
...
public:
    QFirebirdDriverPrivate() : QSqlDriverPrivate(), firebird(0), trans(0)
    { dbmsType = QSqlDriver::Interbase; } // Keeping dbmsType as Interbase to maintain Qt compatibility logic if needed, or could be changed if QSqlDriver allows
    ...
    bool isError(const char *msg, QSqlError::ErrorType typ = QSqlError::UnknownError)
    {
        ...
        QString imsg;
        ISC_LONG sqlcode;
        if (!getFirebirdError(imsg, status, sqlcode))
            return false;

        q->setLastError(QSqlError(QCoreApplication::translate("QFirebirdDriver", msg),
                                  imsg, typ,
    ...
    void initTZMappingCache()
    {
        ...
        if (qry.lastError().isValid()) {
            qCInfo(lcFirebird) << "Table 'RDB$TIME_ZONES' not found - not timezone support available";
            return;
        }
...
public:
    ...
    ~QFirebirdResultPrivate() { cleanup(); }
    ...
    bool isError(const char *msg, QSqlError::ErrorType typ = QSqlError::UnknownError)
    {
        ...
        q->setLastError(QSqlError(QCoreApplication::translate("QFirebirdResult", msg),
                        imsg, typ,
                        sqlcode != -1 ? QString::number(sqlcode) : QString()));
        return true;
    }
...
QFirebirdResultPrivate::QFirebirdResultPrivate(QFirebirdResult *q, const QFirebirdDriver *drv)
    : QSqlCachedResultPrivate(q, drv),
      trans(0),
      localTransaction(!drv_d_func()->firebird),
      stmt(0),
      firebird(drv_d_func()->firebird),
      sqlda(nullptr),
      inda(nullptr),
      queryType(-1)
{
...
bool QFirebirdResultPrivate::writeBlob(qsizetype iPos, const QByteArray &ba)
{
    isc_blob_handle handle = 0;
    ISC_QUAD *bId = (ISC_QUAD*)inda->sqlvar[iPos].sqldata;
    isc_create_blob2(status, &firebird, &trans, &handle, bId, 0, 0);
    if (!isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Unable to create BLOB"),
                 QSqlError::StatementError)) {
        qsizetype i = 0;
        while (i < ba.size()) {
            isc_put_segment(status, &handle, qMin(ba.size() - i, QFirebirdChunkSize),
                            ba.data() + i);
            if (isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Unable to write BLOB")))
                return false;
            i += qMin(ba.size() - i, QFirebirdChunkSize);
        }
    }
...
QVariant QFirebirdResultPrivate::fetchBlob(ISC_QUAD *bId)
{
    isc_blob_handle handle = 0;

    isc_open_blob2(status, &firebird, &trans, &handle, bId, 0, 0);
    if (isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Unable to open BLOB"),
                QSqlError::StatementError))
        return QVariant();

    unsigned short len = 0;
    constexpr auto chunkSize = QFirebirdChunkSize;
    ...
    bool isErr = (status[1] == isc_segstr_eof ? false :
                    isError(QT_TRANSLATE_NOOP("QFirebirdResult",
                                                "Unable to read BLOB"),
                                                QSqlError::StatementError));
...
QVariant QFirebirdResultPrivate::fetchArray(int pos, ISC_QUAD *arr)
{
    QList<QVariant> list;
    ISC_ARRAY_DESC desc;
    ...
    isc_array_lookup_bounds(status, &firebird, &trans, relname.data(), sqlname.data(), &desc);
    if (isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not find array"),
                QSqlError::StatementError))
        return list;
    ...
    isc_array_get_slice(status, &firebird, &trans, arr, &desc, ba.data(), &bufLen);
    if (isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not get array data"),
                QSqlError::StatementError))
        return list;
...
bool QFirebirdResultPrivate::writeArray(qsizetype column, const QList<QVariant> &list)
{
    Q_Q(QFirebirdResult);
    QString error;
    ISC_ARRAY_DESC desc;
    ...
    if (!createArrayBuffer(ba.data(), list,
                           qFirebirdTypeName(desc.array_desc_dtype, sqlvar.sqlscale < 0),
                           0, &desc, error)) {
    ...
    isc_array_put_slice(status, &firebird, &trans, arrayId, &desc, ba.data(), &bufLen);
    return true;
}


bool QFirebirdResultPrivate::isSelect()
{
    char acBuffer[9];
    char qType = isc_info_sql_stmt_type;
    isc_dsql_sql_info(status, &stmt, 1, &qType, sizeof(acBuffer), acBuffer);
    if (isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not get query info"),
                QSqlError::StatementError))
        return false;
...
bool QFirebirdResultPrivate::transaction()
{
    if (trans)
        return true;
    if (drv_d_func()->trans) {
    ...
    isc_start_transaction(status, &trans, 1, &firebird, 0, NULL);
    if (isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not start transaction"),
...
// driver level
bool QFirebirdResultPrivate::commit()
{
    if (!trans)
        return false;
    ...
    trans = 0;
    return !isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Unable to commit transaction"),
                    QSqlError::TransactionError);
}
...
//     qDebug("prepare: %ls", qUtf16Printable(query));
    ...
    createDA(d->sqlda);
    if (d->sqlda == (XSQLDA*)0) {
        qCWarning(lcFirebird) << "QIOFirebirdResult: createDA(): failed to allocate memory";
        return false;
    }
    ...
    isc_dsql_allocate_statement(d->status, &d->firebird, &d->stmt);
    if (d->isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not allocate statement"),
                   QSqlError::StatementError))
        return false;
    isc_dsql_prepare(d->status, &d->trans, &d->stmt, 0,
        const_cast<char*>(query.toUtf8().constData()), FBVERSION, d->sqlda);
    if (d->isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not prepare statement"),
                   QSqlError::StatementError))
        return false;

    isc_dsql_describe_bind(d->status, &d->stmt, FBVERSION, d->inda);
    if (d->isError(QT_TRANSLATE_NOOP("QFirebirdResult",
                    "Could not describe input statement"), QSqlError::StatementError))
        return false;
    if (d->inda->sqld > d->inda->sqln) {
        enlargeDA(d->inda, d->inda->sqld);
        if (d->inda == (XSQLDA*)0) {
            qCWarning(lcFirebird) << "QIOFirebirdResult: enlargeDA(): failed to allocate memory";
            return false;
        }
    ...
    if (d->sqlda->sqld > d->sqlda->sqln) {
        ...
        if (d->isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not describe statement"),
                       QSqlError::StatementError))
            return false;
    }
    initDA(d->sqlda);
...
bool QFirebirdResult::exec()
{
    ...
    if (d->inda) {
        ...
        if (values.count() > d->inda->sqld) {
            qCWarning(lcFirebird) << "QFirebirdResult::exec: Parameter mismatch, expected"_L1
        ...
        for (qsizetype para = 0; para < values.count(); ++para) {
            ...
            if (sqlvar.sqltype & 1) {
                ...
                *(sqlvar.sqlind) = 0;
                ...
                if (QSqlResultPrivate::isVariantNull(val)) {
                    qCWarning(lcFirebird) << "QFirebirdResult::exec: Null value replaced by default (zero)"_L1
            ...
            case SQL_BOOLEAN:
                *((bool*)sqlvar.sqldata) = val.toBool();
                break;
            default:
                    qCWarning(lcFirebird, "QFirebirdResult::exec: Unknown datatype %d",
                              sqltype);
                    break;
            }
        }
    ...
    if (ok) {
        ...
        QString imsg;
        ISC_LONG sqlcode;
        if (getFirebirdError(imsg, d->status, sqlcode) && sqlcode != -501) {
            setLastError(QSqlError(QCoreApplication::translate("QFirebirdResult", "Unable to close statement"),
        ...
        if (d->isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Unable to execute query")))
            return false;
...
bool QFirebirdResult::gotoNext(QSqlCachedResult::ValueCache& row, int rowIdx)
{
    ...
    if (d->isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not fetch next item"),
                   QSqlError::StatementError))
        return false;
    ...
    for (int i = 0; i < d->sqlda->sqld; ++i) {
        ...
        if ((sqlvar.sqltype & 1) && *sqlvar.sqlind) {
            // null value
            QVariant v;
            v.convert(QMetaType(qFirebirdTypeName2(sqlvar.sqltype, sqlvar.sqlscale < 0)));
        ...
        default:
            ...
            qCWarning(lcFirebird, "gotoNext: unknown sqltype: %d", sqltype);
            row[idx] = QVariant();
            break;
        }
    }
...
//        isc_database_info(d->status, &d->firebird, sizeof(sizeInfo), sizeInfo, sizeof(buf), buf);
...
int QFirebirdResult::numRowsAffected()
{
    ...
    case isc_info_sql_stmt_exec_procedure:
        ...
        break;
    default:
        qCWarning(lcFirebird) << "numRowsAffected: Unknown statement type (" << d->queryType << ")";
        return -1;
    }
    ...
    if (d->isError(QT_TRANSLATE_NOOP("QFirebirdResult", "Could not get statement info"),
                   QSqlError::StatementError))
        return -1;
...
QSqlRecord QFirebirdResult::record() const
{
    ...
    for (int i = 0; i < d->sqlda->sqld; ++i) {
        ...
        QSqlField f(QString::fromLatin1(v.aliasname, v.aliasname_length).simplified(),
                    QMetaType(qFirebirdTypeName2(v.sqltype, v.sqlscale < 0)),
                        ...
                        const QString &connOpts)
{
    ...
    for (auto entryView : opts) {
        ...
        // On convertit la vue en QString pour utiliser simplified()
        const QString tmp = entryView.toString().simplified();
        
        // 2. Recherche du caractère '=' (u'=' est le littéral char16_t pour Qt 6)
        const int idx = tmp.indexOf(u'='); 

        if (idx != -1) {
            const QString val = tmp.mid(idx + 1).simplified();
            const QString opt = tmp.left(idx).simplified();

            // 3. Comparaison optimisée :
            // On utilise compare() avec Qt::CaseInsensitive au lieu de créer 
            // une copie majuscule de la chaîne avec toUpper().
            if (opt.compare(u"ISC_DPB_LC_CTYPE", Qt::CaseInsensitive) == 0) {
                encString = val;
            } 
            else if (opt.compare(u"ISC_DPB_SQL_ROLE_NAME", Qt::CaseInsensitive) == 0) {
    ...
    if (isOpen()) {
        ...
        if (d->eventBuffers.size()) {
            ...
            for (i = d->eventBuffers.constBegin(); i != d->eventBuffers.constEnd(); ++i) {
                QFirebirdEventBuffer *eBuffer = i.value();
                eBuffer->subscriptionState = QFirebirdEventBuffer::Finished;
                isc_cancel_events(status, &d->firebird, &eBuffer->eventId);
                qFreeEventBuffer(eBuffer);
            }
            d->eventBuffers.clear();
        }

        isc_detach_database(d->status, &d->firebird);
        d->firebird = 0;
        setOpen(false);
        setOpenError(false);
    }
...
bool QFirebirdDriver::beginTransaction()
{
    ...
    isc_start_transaction(d->status, &d->trans, 1, &d->firebird, 0, NULL);
    return !d->isError(QT_TRANSLATE_NOOP("QFirebirdDriver", "Could not start transaction"),
                       QSqlError::TransactionError);
}
...
bool QFirebirdDriver::commitTransaction()
{
    ...
    d->trans = 0;
    return !d->isError(QT_TRANSLATE_NOOP("QFirebirdDriver", "Unable to commit transaction"),
                       QSqlError::TransactionError);
}
...
bool QFirebirdDriver::rollbackTransaction()
{
    ...
    d->trans = 0;
    return !d->isError(QT_TRANSLATE_NOOP("QFirebirdDriver", "Unable to rollback transaction"),
                       QSqlError::TransactionError);
}
...
QSqlRecord QFirebirdDriver::record(const QString& tablename) const
{
    ...
    while (q.next()) {
        ...
        QSqlField f(q.value(0).toString().simplified(), QMetaType(qFirebirdTypeName(type, hasScale)), tablename);
        if (hasScale) {
...
QSqlIndex QFirebirdDriver::primaryIndex(const QString &table) const
{
    ...
    while (q.next()) {
        QSqlField field(q.value(1).toString().simplified(),
                        QMetaType(qFirebirdTypeName(q.value(2).toInt(), q.value(3).toInt() < 0)),
                        tablename);
...
QVariant QFirebirdDriver::handle() const
{
    ...
    return QVariant(QMetaType::fromType<isc_db_handle>(), &d->firebird);
}
...
static ISC_EVENT_CALLBACK qEventCallback(char *result, ISC_USHORT length, const ISC_UCHAR *updated)
{
    ...
    qMutex()->lock();
    QFirebirdDriver *driver = qBufferDriverMap()->value(result);
    qMutex()->unlock();
...
bool QFirebirdDriver::subscribeToNotification(const QString &name)
{
    Q_D(QFirebirdDriver);
    if (!isOpen()) {
        qCWarning(lcFirebird, "QFirebirdDriver::subscribeFromNotificationImplementation: database not open.");
        return false;
    }

    if (d->eventBuffers.contains(name)) {
        qCWarning(lcFirebird, "QFirebirdDriver::subscribeToNotificationImplementation: already subscribing to '%ls'.",
                  qUtf16Printable(name));
        return false;
    }

    QFirebirdEventBuffer *eBuffer = new QFirebirdEventBuffer;
    eBuffer->subscriptionState = QFirebirdEventBuffer::Starting;
...
bool QFirebirdDriver::unsubscribeFromNotification(const QString &name)
{
    Q_D(QFirebirdDriver);
    if (!isOpen()) {
        qCWarning(lcFirebird, "QFirebirdDriver::unsubscribeFromNotificationImplementation: database not open.");
        return false;
    }

    if (!d->eventBuffers.contains(name)) {
        qCWarning(lcFirebird, "QFirebirdDriver::QFirebirdSubscriptionState not subscribed to '%ls'.",
                  qUtf16Printable(name));
        return false;
    }
...
void QFirebirdDriver::qHandleEventNotification(void *updatedResultBuffer)
{
    Q_D(QFirebirdDriver);
    QMap<QString, QFirebirdEventBuffer *>::const_iterator i;
    for (i = d->eventBuffers.constBegin(); i != d->eventBuffers.constEnd(); ++i) {
        ...
        if (counts[0]) {
            ...
            if (eBuffer->subscriptionState == QFirebirdEventBuffer::Subscribed)
                emit notification(i.key(), QSqlDriver::UnknownSource, QVariant());
            else if (eBuffer->subscriptionState == QFirebirdEventBuffer::Starting)
                eBuffer->subscriptionState = QFirebirdEventBuffer::Subscribed;
            ...
            if (Q_UNLIKELY(status[0] == 1 && status[1])) {
                qCritical("QFirebirdDriver::qHandleEventNotification: could not resubscribe to '%ls'",
...
#include "moc_qsql_firebird_p.cpp"