...
RSLogicActionsManager::RSLogicActionsManager(RSDatabaseAccess* dbAccess, IRSMainWindow* mainWindow, IRSTimesManager* timeManager,
                                             IRSOptionsManager* optionsMgr, IRSGraphView* graphView, IRSFiltersManager* filter,
                                             QObject* parent)
    : QObject(parent)
...
void RSLogicActionsManager::connectSignals()
{
    ...
    if(!connect(signaler, SIGNAL(signal_graphViewStepChanged(int)), this, SLOT(slot_graphViewStepChanged(int))))
...
void RSLogicActionsManager::buildCurves()
{
    ...
    dataComputation.computeFidelityNoiseStepAndInitGraphView(true, m_settings);
