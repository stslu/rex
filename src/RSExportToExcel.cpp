...
RSExportToExcel::RSExportToExcel(
    int trend, double sigma, double noiseFactor, int stepDays, int nbSteps, QObject *parent)
    : QObject(parent)
    , m_book(0)
    , m_trend(trend)
    , m_sigma(sigma)
                               ...
                               QString &info)
{
    ...
    if (m_filePath.isEmpty()) {
        QString fileName = QString("RexExport_%1")
    ...
    if (!canOpen) {
        QMessageBox::warning(NULL,
                             QObject::tr("Failed to save excel file"),
                             QObject::tr("Failed to write excel data to file %1: %2.")
                                 .arg(m_filePath)
                                 .arg(fileCheck.errorString()));
        fileCheck.close();
        RSLogger::instance()->info(Q_FUNC_INFO,
                                   "End. canOpen = fileCheck.open( QFile::WriteOnly ) = FALSE ");
        return;
    }
...
void RSExportToExcel::createDataSheet(const QString &sheetName,
                                      const QString &title,
...
void RSExportToExcel::createDataLineGroup(
    Sheet *sheet, int row, int col, int nbCol, const QString &data, Format *format)
{
    //    RSLogger::instance()->info(Q_FUNC_INFO,"Start");
